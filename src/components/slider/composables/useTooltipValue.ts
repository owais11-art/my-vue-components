...
export function useTooltipValue(sliderValue: ModelRef<number | TRangeValues>, minRange: Ref<number>, maxRange: Ref<number>, stepsCompleted: Ref<number | number[]>, min: number, max: number, range: boolean, isVerticalSlider: boolean): TUseTooltipValueReturn{
    return function (tooltip: string="right"): number{
        if(!range) return min && max ? min + (stepsCompleted.value as number): sliderValue.value as number
